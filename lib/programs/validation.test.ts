...
import { PROGRAM_ENDS_ON_ERROR_KEY, validateProgramDates } from "./validation";
...
describe("validateProgramDates", () => {
  ...
  it("rechaza fin anterior a inicio con la clave del mensaje del backend", () => {
    expect(validateProgramDates("2026-06-30", "2026-01-01")).toBe(PROGRAM_ENDS_ON_ERROR_KEY);
  });
...
describe("PROGRAM_ENDS_ON_ERROR_KEY", () => {
  it("el catálogo es mantiene el texto literal del contrato del backend", () => {
    expect(es.programs.errors.endsOnBeforeStartsOn).toBe(
