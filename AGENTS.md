...
- `axe-core` vía `vitest-axe` (`vitest.setup.ts` registra
  ...
  `plataforma/entidades/[id]`) son **27 páginas y 8 componentes**:
  ...
  y `components/plataforma/AyudaPendienteList` — estos ocho cubren por
  dentro lo que sus páginas (`guardia`, la ayuda de pantalla y el
  selector de idioma de las tres cabeceras de área, el menú lateral, la
  comparativa de los dos dashboards, los dos `informes` y el propio mapa
  de Territorio) no comprueban siempre por fuera.
