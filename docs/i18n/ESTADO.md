...
| 2026-09-20 | Bloque 1 «Administraciones multinivel y territorio» (Tareas 1-8 de `docs/superpowers/plans/2026-09-19-territorio-panel.md`) | `messages/{es,eu,ca}.json` — unas 80 claves hoja nuevas: `metrics.map.*` (7, leyenda y `aria-label` del mapa), `metrics.placeSheet.*` (11, ficha de municipio, incluida la nota de invariante 1 de «nunca nombres»), `metrics.export.scopeTerritorio` (el resto de `scope*` ya existía desde W2); `paraguas.territorio.*` (9), `paraguas.redFinanciada.*` (2, cabecera y «sin acceso» — el resto de textos del dashboard son los mismos de `metrics.dashboard.*` que ya traducía la Tarea 5), `paraguas.inicio.*` (6, las dos tarjetas-resumen y sus dos enlaces); `plataforma.sede.*` (10, `SedeSelector`) y `plataforma.territorio.*` (21, `TerritorioForm`, incluidas las etiquetas de los cuatro niveles administrativos y los tres tipos de territorio); `errors.{metrics,compare,export}.sinTerritorio` (el mensaje literal del 409, igual en los tres), `errors.placeSheet.*` (4), `errors.places.*` (2) y `errors.setOrganizationTerritory.*` (3); más los renombres de `menu.entidad.biblioteca`/`menu.plataforma.suscripciones` (antes `recursos`/`contratos`, mismo texto en el resto de idiomas salvo la palabra que cambia), `menu.paraguas.{territorio,redFinanciada}` (nuevas, `inicio`/`informes` ya existían), `pages.paraguas.{territorio,redFinanciada}` y `help.paraguas.{territorio,redFinanciada}` (el registro de ayuda por pantalla pasa de 32 a 34 entradas). | borrador sin revisar | borrador sin revisar | `es` es literal a los textos del brief/spec de diseño de este bloque, sin reescrituras de contenido — ninguna concatenación nueva que convertir a plural ICU en este bloque. `TerritoryMap`/`PlaceSheetPanel` son los dos componentes nuevos con catálogo propio; `metrics.table.actionsHeader` que anticipaba el plan original **no llegó a crearse** — la columna de acciones de `MetricsTable` reutiliza `common.actions` (ya traducido desde la Tarea 2), así que no hay clave duplicada que mantener. |
